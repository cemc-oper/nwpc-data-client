...
@test "test grapes_gfs_gmf/grib2/orig runtime" {
  expected_result="/g2/nwp_pd/NWP_PST_DATA/GMF_GRAPES_GFS_POST/togrib2/output_togrib2/${check_date_time}/modelvar${check_date_time}003.grb2"
  if [ -f "${expected_result}" ]; then
    run "${NWPC_DATA_CLIENT_PROGRAM}" local \
        --location-level=runtime \
        --data-type=grapes_gfs_gmf/grib2/modelvar \
        "${check_date_time}" 3h
...
@test "test grapes_gfs_gmf/grib2/orig archive" {
  expected_result="/g1/COMMONDATA/OPER/NWPC/GRAPES_GFS_GMF/Prod-grib/${check_data_time_4dvar}/MODELVAR/modelvar${check_date_time}003.grb2"
  if [ -f "${expected_result}" ]; then
    run "${NWPC_DATA_CLIENT_PROGRAM}" local \
