...
@test "test grapes_gfs_gda/bin/postvar runtime/archive" {
  expected_result="/g2/nwp/GRAPES_GFS/DATA/DATABAK/NWP_GDAS/FCST_results/postvar${check_date}${hour_4dv}_003"
  if [ -f "${expected_result}" ]; then
...
@test "test grapes_gfs_gda/bin/postvar runtime/archive with config" {
  expected_result="/g2/nwp/GRAPES_GFS/DATA/DATABAK/NWP_GDAS/FCST_results/postvar${check_date}${hour_4dv}_003"
  if [ -f "${expected_result}" ]; then
    result=$(${NWPC_DATA_CLIENT_PROGRAM} local \
        --location-level=runtime/archive \
