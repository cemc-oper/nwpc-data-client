...
@test "test cma_gfs_gmf/current/grib2/modelvar runtime" {
  expected_result="/g2/op_post/OPER/WORKDIR/NWP_CMA_GFS_GMF_POST_DATA/${check_date_time}/data/output/grib2_orig/modelvar${check_date_time}003.grb2"
  ...
  run "${NWPC_DATA_CLIENT_PROGRAM}" local \
    ...
    --data-type=cma_gfs_gmf/current/grib2/modelvar \
    --start-time "${check_date_time}" \
    --forecast-time 3h
  assert_output ${expected_result}
}

@test "test cma_gfs_gmf/current/grib2/modelvar archive" {
  expected_result="/g3/COMMONDATA/OPER/CEMC/GFS_GMF/Prod-grib/${check_date_time}/MODELVAR/modelvar${check_date_time}003.grb2"
  if [ ! -f "${expected_result}" ]; then
    skip "data is not available: ${expected_result}"
  fi
  run "${NWPC_DATA_CLIENT_PROGRAM}" local \
