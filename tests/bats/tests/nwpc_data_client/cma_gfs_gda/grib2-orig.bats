...
@test "test cma_gfs_gda/current/grib2/orig runtime" {
  expected_result="/g2/op_post/OPER/WORKDIR/NWP_CMA_GFS_GDA_POST_DATA/${check_date_time}/data/output/grib2_orig/gda.gra.${check_date_time}000.grb2"
  ...
  run "${NWPC_DATA_CLIENT_PROGRAM}" local \
    ...
    --data-type=cma_gfs_gda/current/grib2/orig \
...
@test "test cma_gfs_gda/current/grib2/orig archive" {
  expected_result="/g3/COMMONDATA/OPER/CEMC/GFS_GDA/Prod-grib/${check_date_time}/ORIG/gda.gra.${check_date_time}000.grb2"
  if [ ! -f "${expected_result}" ]; then
    skip "data is not available: ${expected_result}"
  fi
  run "${NWPC_DATA_CLIENT_PROGRAM}" local \
...
@test "test cma_gfs_gda/current/grib2/orig archive with config" {
  expected_result="/g3/COMMONDATA/OPER/CEMC/GFS_GDA/Prod-grib/${check_date_time}/ORIG/gda.gra.${check_date_time}000.grb2"
  if [ ! -f "${expected_result}" ]; then
    skip "data is not available: ${expected_result}"
  fi
